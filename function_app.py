...
def map_fields_with_azure_openai(text_output, fields_geo, phrases_geo):
    system_instructions = (
        ...
        "     - 'Attorney for (name):' (a blank where the filer writes WHO they represent), or a caption like\n"
        "       'Name of party represented', captures the represented party = the CLIENT. Map it to Contacts\n"
        "       (`{721E1}`), Client instance (`{40EB6}`), Full Name (`{166FD}`). The client is usually named\n"
        "       NOWHERE else on the form, so this is the primary place the client is captured — do not skip it.\n"
        "     - DISTINGUISH that from a field that holds an ATTORNEY's name for a specific party — captions like\n"
        "       'Attorney for Plaintiff', 'Attorney for Defendant', \"Plaintiff's Attorney\", \"Defendant's\n"
        "       Attorney\". Those contain the attorney's name, NOT the client. Map them to the matching\n"
        "       attorney instance from the Context (e.g. Plaintiff's Attorney `{18EA6}`, Defendant's Attorney\n"
        "       `{86EEE}`, Attorney for Plaintiff/Petitioner `{6EC71}`, Attorney for Defendant/Respondent\n"
        "       `{0664C}`), Full Name (`{166FD}`) — never the Client.\n"
        "  2. **Case-level data**\n"
