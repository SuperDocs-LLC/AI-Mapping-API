...
def map_fields_with_azure_openai(text_output, fields_geo, phrases_geo):
    system_instructions = (
        ...
        "     - A field captioned 'Attorney for (name)', 'Attorney for', 'Attorney for Plaintiff/Defendant',\n"
        "       'Name of party represented', or similar captures the name of the party the FILING ATTORNEY\n"
        "       REPRESENTS — that party is the CLIENT. Map it to the Contacts group (`{721E1}`), Client\n"
        "       instance (`{40EB6}`), expression Full Name (`{166FD}`). The client is usually named NOWHERE\n"
        "       else on the form, so this is the primary place the client is captured — do not skip it.\n"
        "  2. **Case-level data**\n"
